...
export class Shelfmark {
  ...
  constructor(value: string) {
    ...
    this.#suffix = [this.#suffix[0].toUpperCase(), this.#suffix.slice(1).toLowerCase()].join("");
  }

  get class(): string {
    const endOfTopic = [...this.#main].findIndex((c) => c >= "0" && c <= "9");
...
export const stacks: Stack[] = stackConfig.map((stack) => {
  return {
    ...
    segments: stack.segments.map((segment) => {
      return {
        ...segment,
        marks: segment.marks.map((mark) => new Shelfmark(mark)) as [Shelfmark, Shelfmark],
      };
    }),
  };
});
